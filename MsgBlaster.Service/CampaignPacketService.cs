...
{
    public class CampaignPacketService
    {
            ...
            try
            {
                ...
                if (ValidatePacketAgainstSchema(packet)) // Check xml file validation.
                {
                    if (AllowToProcessClient(CampaignDTO))
                    {
                        ...
                        {
                            requiredCredit += MessageCount(MsgCorrect(messages[i].InnerText.TrimEnd()), CampaignDTO);
                        }
                        ...
                        //{
                            ...
                            requiredCredit = SettingDTO.NationalCampaignSMSCount * requiredCredit; ;
                            ...
                            try
                            {
                               ...
                               // CampaignDTONew.IsSent = true;
                                ...
                                if (CampaignDTONew.GroupId == 0)
                                {
                                    CampaignDTONew.GroupId = null;
                                }
                                
                                 CampaignDTONew.RequiredCredits = requiredCredit;
                                 CampaignDTONew.CreditsDiffrence = CampaignDTO.ConsumedCredits - requiredCredit;
                                //CampaignService.EditCampaignFromBackend(CampaignDTONew);
                                ...
                                ////Check CreditsDiffrence is -ve
                                 
                                if (CampaignDTONew.ConsumedCredits != requiredCredit)                                    
                                {                                      
                                     
                                    if (CampaignDTONew.CreditsDiffrence < 0)                                        
                                    {
                                        //// deduct clients balance
                                        ClientDTO ClientDTOUpdate = new ClientDTO();
                                        ClientDTOUpdate = ClientDTO;
                                        ClientDTOUpdate.SMSCredit = ClientDTOUpdate.SMSCredit - (-(CampaignDTONew.CreditsDiffrence));
                                       
                                        ClientService.Edit(ClientDTOUpdate);
                                       
                                        CampaignDTONew.RequiredCredits = requiredCredit;
                                        //CampaignDTONew.CreditsDiffrence = CampaignDTO.ConsumededCredits - requiredCredit;
                                        CampaignDTONew.IsReconcile = true;
                                        CampaignDTONew.ReconcileDate = System.DateTime.Now;
                                        CampaignService.EditCampaignFromBackend(CampaignDTONew);                                        
                                    }                                       
                                    else if (CampaignDTONew.CreditsDiffrence > 0) 
                                    {
                                        ////Add clients balance
                                        ClientDTO ClientDTOUpdate = new ClientDTO();
                                        ClientDTOUpdate = ClientDTO;
                                    ...
                                    {
                                        //If CreditsDiffrence 0 then just modify IsReconcile and reconcile date
                                        CampaignDTONew.RequiredCredits = requiredCredit;
                                        CampaignDTONew.IsReconcile = true;
                                        CampaignDTONew.ReconcileDate = System.DateTime.Now;
                                        CampaignService.EditCampaignFromBackend(CampaignDTONew);
                                ...
                                else if (CampaignDTONew.ConsumedCredits == requiredCredit)                                    
                                {                                      
                                    //If CreditsDiffrence 0 then just modify IsReconcile and reconcile date
                                    CampaignDTONew.RequiredCredits = requiredCredit;
                                    CampaignDTONew.IsReconcile = true;
        ...
        //
        public static int AddMsgRecipToXmlpacket(string message, ArrayList recipients, string xmlpacket, MsgInformationDTO _oMsginfo, int recipientsnumberscount, CampaignDTO CampaignDTO)
        {
            //string xmlpacket = null;
            ...
            xmlpacket += "</packet>";
            _oMsginfo.xmlpacket = xmlpacket;
            int requiredCreditTosendmsg = MessageCount(message, CampaignDTO);
            _oMsginfo.RequiredCredits = requiredCreditTosendmsg;
            Byte[] array = Serializeobject(_oMsginfo);
            Byte[] checksum = new MD5CryptoServiceProvider().ComputeHash(array); // calculate checksum for validation
            ...
            /////////////////////////////////////////////////////////////////
            return MessageCount(message, CampaignDTO);
        }
        ...
        /// Removing the mailmarge microws from Message with specific  value 
        /// </summary>
        /// <param name="mailmargeMessage">Message which contains macros</param>
        /// <param name="recipientInfo">Contact information</param>
        /// <returns>Returns modified message </returns>
        private static string FormatMessageTextMicrows(string mailmargeMessage, DataView recipientInfo)
        {
            ...
            if (dv.Count > 0)
            {
                foreach (DataRow drForm in dv.ToTable().Rows)
                {
                    ...
                    string FirstName = "";
                    FirstName = drForm["FirstName"].ToString();// CommonService.GetFirstname(Name);

                    string LastName = "";
                    LastName = drForm["LastName"].ToString();// CommonService.GetLastname(Name); 

                    
                    msgtext = msgtext.Replace("[FirstName]", FirstName);
                    msgtext = msgtext.Replace("[LastName]", LastName);

                    if (drForm["BirthDate"].ToString() != "")
                    {
                        DateTime BirthDate = Convert.ToDateTime(drForm["BirthDate"]);
                        string bdate = BirthDate.ToString("dd-MMM-yy");
                        msgtext = msgtext.Replace("[BirthDate]", bdate);
                    }
                    else { msgtext = msgtext.Replace("[BirthDate]", ""); }

                    if (drForm["AnniversaryDate"].ToString() != "")
                    {
                        DateTime AnniversaryDate = Convert.ToDateTime(drForm["AnniversaryDate"]);
                        string adate = AnniversaryDate.ToString("dd-MMM-yy");
                        msgtext = msgtext.Replace("[AnniversaryDate]", adate);
                    }
                    else { msgtext = msgtext.Replace("[AnniversaryDate]", ""); }


                    if (drForm["Email"].ToString() != "")
                    {
                        string Email = drForm["Email"].ToString();

                        msgtext = msgtext.Replace("[Email]", Email);
                    }
                    else { msgtext = msgtext.Replace("[Email]", ""); }

                    if (drForm["MobileNumber"].ToString() != "")
                    {
                        string MobileNumber = drForm["MobileNumber"].ToString();

                        msgtext = msgtext.Replace("[MobileNumber]", MobileNumber);
                    }
                    else { msgtext = msgtext.Replace("[MobileNumber]", ""); }


                    if (drForm["Gender"].ToString() != "")
                    {
                        string Gender = drForm["Gender"].ToString();
                        if (Gender == "0")
                        {
                            Gender = "Male";
                        }
                        ...
                        msgtext = msgtext.Replace("[Gender]", Gender);
                    }
                    else { msgtext = msgtext.Replace("[Gender]", ""); }
                ...
                if (!string.IsNullOrEmpty(MacrosList[i].ToString()))
                {
                    switch (i.ToString())
                    {
                        ...
                        case "4":
                            ...
                            break;
                        case "5":
                            valueList.Add("Label6");
                            break;
                        case "6":
                            valueList.Add("Label7");
                            break;
                        case "7":
                            valueList.Add("Label8");
                            break;
                        case "8":
                            valueList.Add("Label9");
                            break;
                    }
                }
            }
